	...
	return (
		...
		>
			...
			<div className='relative aspect-[3/4] w-full bg-gray-50'>
				...
				{/* Format Badge */}
				<div className='absolute top-3 left-3 bg-amber-600 text-white text-[10px] xs:text-xs font-bold px-2 sm:px-3 py-0.5 sm:py-1 rounded-full shadow-sm'>
					{category?.includes("Hardcover") ? "Hardcover" : "Paperback"}
				</div>
					...
					{cartItemCount > 0 ? (
						<div className='flex items-center justify-between bg-gray-50 rounded-full px-3 sm:px-4 py-1.5 sm:py-2 border border-primaryColor-200'>
							...
							<span className='text-xs sm:text-sm font-medium'>
								{cartItemCount} {cartItemCount > 1 ? "Copies" : "Copy"}
							</span>
							<button
								onClick={(e) => {
									e.stopPropagation();
